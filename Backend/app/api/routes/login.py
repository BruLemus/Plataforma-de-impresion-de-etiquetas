...
def login(ciudad: str, tipo: str, nombre: str = Form(...), contrasena: str = Form(...), db: Session = Depends(get_db)):
    ...
    # Generar token para ambos
    token_data = {"user_id": user.id, "tipo": tipo}
...
# ---------------------------
def get_current_user(role: str, token: str = Header(...), db: Session = Depends(get_db)):
    if not token:
    ...
    try:
        ...
        if not user_id or token_role != role:
            raise HTTPException(status_code=401, detail="Token inválido o rol incorrecto")
    except JWTError:
