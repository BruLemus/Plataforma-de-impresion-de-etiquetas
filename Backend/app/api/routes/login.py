...
def get_current_user(role: str, token: str = None, db: Session = Depends(get_db)):
    if role == "practicante":
        user = db.query(UserPracticante).first()  # Ajustar según login real
        if not user:
            raise HTTPException(status_code=401, detail="Practicante no encontrado")
        return user
    if not token:
        raise HTTPException(status_code=401, detail="Token requerido para coordinador")
    try:
    ...
    if not user:
        raise HTTPException(status_code=401, detail="Coordinador no encontrado")
    return user
